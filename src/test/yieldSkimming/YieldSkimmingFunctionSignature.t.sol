...
import {YieldSkimmingSetup, IERC20, IStrategyInterface} from "./YieldSkimmingSetup.sol";

contract YieldSkimmingFunctionSignatureTest is YieldSkimmingSetup {
    ...
    function test_functionCollisions() public {
        ...
        assertEq(strategy.pricePerShare(), 10 ** 18, "pps"); // YieldSkimming uses 18 decimals
        assertTrue(!strategy.isShutdown());
        assertEq(
            strategy.symbol(),
            string(abi.encodePacked("ys", "WSTETH")), // Using WSTETH symbol
            "symbol"
        );
        assertEq(strategy.decimals(), 18, "decimals"); // YieldSkimming uses 18 decimals
        ...
        vm.expectRevert("!management");
        strategy.setDragonRouter(user); // Changed from setPerformanceFeeRecipient
        vm.expectRevert("!management");
        ...
        strategy.setDragonRouter(address(strategy)); // Changed from setPerformanceFeeRecipient
        vm.expectRevert("too long");
        strategy.setProfitMaxUnlockTime(type(uint256).max);
        vm.stopPrank();

        // Mint some shares to the user
        airdrop(IERC20(address(strategy)), user, wad);
